...
# Keystore2 bootloop prevention
check_scm_status() {
    if dmesg | grep -q "scm_call failed"; then
        log_print 1 "SCM call failures detected, applying workarounds..."
        
        # Set properties to bypass problematic SCM calls
        resetprop vendor.qti.scm.disable_mem_protection 1
        resetprop ro.vendor.qti.va_aosp.support 1
        
        # Use alternative keymaster if primary fails
        if [ ! -f /vendor/bin/hw/android.hardware.keymaster@4.0-service-qti ]; then
            log_print 1 "Primary keymaster not found, using fallback"
            resetprop keymaster_ver 3.0
        fi
    fi
}
